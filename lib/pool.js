...
function Miner(rewardType, childRewardType, id, childPoolIndex, login, pass, ip, port, agent, childLogin, startingDiff, noRetarget, pushMessage, sendReply){
    ...
    } else if (pass.length > 0 && !pass.includes('@')) {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, pool){
    ...
    if (rewardType === 'solo') {
        redisCommands.push(['hincrby', `${coin}:workers:${login}`, 'soloShares', job.difficulty]);
    }
    ...
    if (blockCandidate){
        ...
        if (rewardType === 'solo') {
            ...
            redisCommands.push(['hget', `${coin}:workers:${login}`, 'soloShares']);
        }
    }
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate) {
            ...
            let redisCommands2 = [];
            if (rewardType === 'solo') {
                redisCommands2.push(['hdel', `${coin}:workers:${login}`, 'soloShares']);
                redisCommands2.push(['hincrby', `${coin}:workers:${login}`, 'soloBlocksFound', 1]);
            }

            if (redisCommands2.length > 0) {
                redisClient.multi(redisCommands2)
                    .exec(function (err, replies) {
                        if (err) {
                            log('error', logSystem, 'Failed to insert/edit share data in redis %j \n %j', [err, redisCommands2]);
                            return;
                        }
                        // log('info', logSystem, `Pplns shares for round ${job_height} successfully inserted to ${coin}:shares:round${job_height}`);
                    })
            }
...
function processShare(miner, job, blockTemplate, params){
    ...
    let childBlockTemplate = blockTemplate.childBlockTemplate;
